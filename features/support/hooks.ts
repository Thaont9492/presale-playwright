 ...
 * Paths, URLs, and default values for the test environment
 */
const STORAGE_STATE_PATH = path.join(__dirname, '../../.auth/storage-state.json');
const SHOPIFY_URL = process.env.SHOPIFY_URL || 'https://snap-presale-check.myshopify.com/';
const SHOPIFY_PASSWORD = process.env.SHOPIFY_PASSWORD || 'aothao';
 ...
 * Creates directories and launches the browser
 */
BeforeAll(async function() {
  try {
    // Create auth directory for storing login state
    const authDir = path.dirname(STORAGE_STATE_PATH);
    if (!fs.existsSync(authDir)) {
      fs.mkdirSync(authDir, { recursive: true });
    }
    
    // Create screenshots directory for failure captures
...
/**
 * AfterAll hook - Runs once after all tests complete
 ...
 * Sets up browser context, handles authentication, and creates a new page
 */
 ...
 * Captures traces, screenshots on failure, and cleans up the page
 */
